...
public class WardrobeManagerTester {
  ...
  public static boolean testClothingWear() {
    Clothing cloth = new Clothing("Shirt", "Gucci");
    try{
      cloth.wearClothing(0, 10, 12);
      return false;
    } catch(IllegalArgumentException e){
      if(e.getMessage() == null || e.getMessage().isBlank()){
        return false;
    ...
    cloth.wearClothing(2020, 20, 10);
    if(cloth.getNumOfTimesWorn() != 1){
      System.out.println("Number of times worn did not increase");
      return false;
    }
    LocalDate date = LocalDate.of(2020, 2, 10);
    if(!(cloth.getLastWornDate().equals(date))){
      System.out.println("The date last worn does not match");
      return false;
    }
